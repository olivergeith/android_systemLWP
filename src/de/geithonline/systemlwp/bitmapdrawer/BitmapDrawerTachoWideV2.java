...
public class BitmapDrawerTachoWideV2 extends BitmapDrawer {
	...
	public BitmapDrawerTachoWideV2() {
	}
	...
	private void drawSkala(final int level) {
		...
		// bgPaint.setShader(new LinearGradient(0, 0, bWidth / 2, 0,
		// Color.BLACK, Color.DKGRAY, Shader.TileMode.MIRROR));

		// Fläche
		bitmapCanvas.drawRect(getRectForOffset(offset), bgPaint);

		// Balken
		// final RectF zr = getRectForOffset(offset);
		// final Paint battPaint = Settings.getBatteryPaint(level);
		// // battPaint.setAlpha(64);
		// battPaint.setShader(new LinearGradient(0, 0, bWidth / 2, 0,
		// ColorHelper.darker(battPaint.getColor()), battPaint.getColor(),
		// Shader.TileMode.MIRROR));
		// zr.left = offset + rahmenDicke + rand;
		// zr.right = w;
		// zr.bottom = offset + rahmenDicke + fontSizeScala + offset;
		// bitmapCanvas.drawRect(zr, battPaint);
