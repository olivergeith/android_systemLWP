...
package de.geithonline.systemlwp.billinghelper;
...
public class IabHelper {
	...
	public void launchPurchaseFlow(final Activity act, final String sku, final String itemType, final int requestCode,
		...
		try {
			...
			final Bundle buyIntentBundle = mService.getBuyIntent(3, mContext.getPackageName(), sku, itemType, extraData);
...
/**
	...
	void flagStartAsync(final String operation) {
		if (mAsyncInProgress) {
			...
			// mAsyncOperation
			// + ") is in progress.");
			logError("Can't start async operation (" + operation + ") because another async operation(" + mAsyncOperation
					+ ") is in progress.");
			return;
		}
	...
	int querySkuDetails(final String itemType, final Inventory inv, final List<String> moreSkus) throws RemoteException, JSONException {
